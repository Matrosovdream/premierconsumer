...
Class PMS_IN_ProRate {
    ...
    public function modify_payment_amount( $payment_data, $checkout_data ){
        ...
        if( !isset( $_GET['subscription_id'] ) || !isset( $payment_data['subscription_plan_id'] ) || !isset( $checkout_data['form_location'] ) || !in_array( $checkout_data['form_location'], array( 'upgrade_subscription', 'downgrade_subscription' ) ) )
            return $payment_data;

        $prorated_discount_data = $this->get_prorated_subscription_discount( absint( $_GET['subscription_id'] ), $payment_data['subscription_plan_id'] );
    ...
    public function log_prorate_discounts_on_payments( $payment_gateway_data, $payment_settings ){
        ...
        if( !isset( $_GET['subscription_id'] ) || !isset( $payment_gateway_data['subscription_plan_id'] ) || !isset( $payment_gateway_data['form_location'] ) )
        ...
        $prorated_discount_data = $this->get_prorated_subscription_discount( absint( $_GET['subscription_id'] ), $payment_gateway_data['subscription_plan_id'] );
