 ...
 *
 */
function pms_in_pp_settings_page_payment_gateway_paypal_title( $title ) {

    return __( 'PayPal', 'paid-member-subscriptions' );

}
add_filter( 'pms_settings_page_payment_gateway_paypal_title', 'pms_in_pp_settings_page_payment_gateway_paypal_title' );
 ...
 *
 */
function pms_in_pp_settings_gateway_paypal_express_extra_fields( $options ) {
...
add_action( 'pms_settings_page_payment_gateway_paypal_extra_fields', 'pms_in_pp_settings_gateway_paypal_express_extra_fields' );
...
if( ! function_exists('pms_in_settings_gateway_paypal_extra_fields') ) {
    ...
    function pms_in_settings_gateway_paypal_extra_fields( $options ) {
        ...
        foreach( $fields as $field_slug => $field_details ) {
            ...
            if ( defined( 'PMS_VERSION' ) && version_compare( PMS_VERSION, '1.7.8' ) == -1 )
                echo '<input id="paypal-' . esc_attr( str_replace('_', '-', $field_slug ) ) . '" type="text" name="pms_settings[payments][gateways][paypal][' . esc_attr( $field_slug ) . ']" value="' . ( isset($options['payments']['gateways']['paypal'][$field_slug]) ? esc_attr( $options['payments']['gateways']['paypal'][$field_slug] ) : '' ) . '" class="widefat" />';
            else
 ...
 *
 */
function pms_in_paypal_express_add_payment_gateway_admin_subscription_fields( $subscription_id = 0, $gateway_slug = '', $gateway_details = array() ) {
    ...
    $billing_agreement_id = ( ! empty( $_POST['_paypal_billing_agreement_id'] ) ? sanitize_text_field( $_POST['_paypal_billing_agreement_id'] ) : $billing_agreement_id );


    // PayPal Billing Agreement ID
    echo '<div class="pms-meta-box-field-wrapper">';

        echo '<label for="pms-subscription-paypal-billing-agreement-id" class="pms-meta-box-field-label">' . esc_html__( 'PayPal Billing Agreement ID', 'paid-member-subscriptions' ) . '</label>';
...
add_action( 'pms_view_add_new_edit_subscription_payment_gateway_extra', 'pms_in_paypal_express_add_payment_gateway_admin_subscription_fields', 10, 3 );
 ...
 *
 */
function pms_in_paypal_express_validate_subscription_data_admin_fields( $admin_notices = array() ) {

    // Validate the billing agreement id
    if( ! empty( $_POST['_paypal_billing_agreement_id'] ) ) {

        if( false === strpos( sanitize_text_field( $_POST['_paypal_billing_agreement_id'] ), 'B-' ) )
...
add_filter( 'pms_submenu_page_members_validate_subscription_data', 'pms_in_paypal_express_validate_subscription_data_admin_fields' );
 ...
 *
 */
function pms_in_paypal_express_save_payment_gateway_admin_subscription_fields( $subscription_id = 0 ) {
...
add_action( 'pms_member_subscription_insert', 'pms_in_paypal_express_save_payment_gateway_admin_subscription_fields' );
add_action( 'pms_member_subscription_update', 'pms_in_paypal_express_save_payment_gateway_admin_subscription_fields' );
