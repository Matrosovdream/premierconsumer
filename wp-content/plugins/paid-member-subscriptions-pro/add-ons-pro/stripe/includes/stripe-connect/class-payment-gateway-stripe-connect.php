...
Class PMS_IN_Payment_Gateway_Stripe_Connect extends PMS_Payment_Gateway {
    ...
    public function init() {
        ...
        if( !is_admin() ) {
            ...
            if( !pms_in_stripe_check_filter_from_class_exists( 'pms_update_payment_method_stripe_connect', get_class($this), 'update_customer_payment_method' ) )
                add_action( 'pms_update_payment_method_stripe_connect', array( $this, 'update_customer_payment_method' ) );

            // Add Form Fields placeholder
            if( !pms_in_stripe_check_filter_from_class_exists( 'pms_output_form_field_stripe_placeholder', get_class($this), 'output_form_field_stripe_placeholder' ) )
    ...
    public function output_form_field_stripe_placeholder( $field = array() ) {
        ...
        if( pms_in_stripe_connect_get_account_status() ){
    ...
    private function add_application_fee( $args ){
        ...
        $account_country      = pms_in_stripe_connect_get_account_country();
    ...
    public function wppb_handle_failed_payment( $content ){
        ...
        return pms_in_stripe_error_message( $content, 1, $this->payment_id );
