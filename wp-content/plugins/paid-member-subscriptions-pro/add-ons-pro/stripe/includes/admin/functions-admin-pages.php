...
function pms_in_add_settings_content_stripe( $options ) {
	...
	if( in_array( 'stripe_intents', $options['active_pay_gates'] ) || in_array( 'stripe', $options['active_pay_gates'] ) || in_array( 'stripe_connect', $options['active_pay_gates'] ) ) :
			...
			if( in_array( 'stripe_connect', $options['active_pay_gates'] ) ) :
				...
				echo '<div class="pms-stripe-connect__gateway-settings">';
					...
					if( !empty( $account ) ){
						...
						$connection_status = pms_in_stripe_connect_get_account_status();
						...
						if( is_array( $connection_status ) ){
							...
							if( empty( $serial_number ) || $serial_number_status != 'valid' )
						...
						echo '<a href="'. esc_url( $stripe_connect_link ) .'" class="pms-stripe-connect__button"><img src="' . esc_attr( PMS_IN_STRIPE_PLUGIN_DIR_URL ) . 'assets/img/stripe-connect.png" /></a><br>';
 ...
 *
 */
function pms_in_stripe_add_payment_gateway_admin_subscription_fields( $subscription_id = 0, $gateway_slug = '', $gateway_details = array() ) {
...
add_action( 'pms_view_add_new_edit_subscription_payment_gateway_extra', 'pms_in_stripe_add_payment_gateway_admin_subscription_fields', 10, 3 );
 ...
 *
 */
function pms_in_stripe_validate_subscription_data_admin_fields( $admin_notices = array() ) {
...
add_filter( 'pms_submenu_page_members_validate_subscription_data', 'pms_in_stripe_validate_subscription_data_admin_fields' );
 ...
 *
 */
function pms_in_stripe_save_payment_gateway_admin_subscription_fields( $subscription_id = 0 ) {
...
add_action( 'pms_member_subscription_insert', 'pms_in_stripe_save_payment_gateway_admin_subscription_fields' );
add_action( 'pms_member_subscription_update', 'pms_in_stripe_save_payment_gateway_admin_subscription_fields' );
...
function pms_in_stripe_add_currencies( $currencies ){
...
add_filter( 'pms_currencies', 'pms_in_stripe_add_currencies' );
