...
Class PMS_IN_Payment_Gateway_Stripe_Payment_Intents extends PMS_IN_Payment_Gateway_Stripe_Legacy {
    /**
    ...
    public function init() {
        ...
        if( !in_array( 'stripe_intents', pms_get_active_payment_gateways() ) )
            return;
        ...
        // Set API secret key
        $api_credentials  = pms_in_get_stripe_api_credentials();
        ...
        if( !pms_in_stripe_check_filter_from_class_exists( 'pms_get_output_payment_gateways', get_class($this), 'field_ajax_nonces' ) )
        ...
        if( !pms_in_stripe_check_filter_from_class_exists( 'pms_get_output_payment_gateways', get_class($this), 'field_publishable_key' ) )
            add_filter( 'pms_get_output_payment_gateways', array( $this, 'field_publishable_key' ), 10, 2 );

        // Add PK to Update Payment method form
        if( !pms_in_stripe_check_filter_from_class_exists( 'pms_update_payment_method_form_bottom', get_class($this), 'update_payment_form_field_publishable_key' ) )
        ...
        if( !pms_in_stripe_check_filter_from_class_exists( 'pms_update_payment_method_form_bottom', get_class($this), 'field_update_payment_method_nonce' ) )
        ...
        if( !pms_in_stripe_check_filter_from_class_exists( 'pms_update_payment_method_stripe_intents', get_class($this), 'update_customer_payment_method' ) )
        ...
        if( !empty( $this->customer_id ) && !empty( $this->stripe_token ) ) {
            try {
                ...
                if( !empty( $member_subscription->subscription_plan_id ) ){
                    ...
                    if($member_subscription->subscription_plan_id == 17015 || $member_subscription->subscription_plan_id == 19950 || $member_subscription->subscription_plan_id == 19953){
                        $s_key = $this->secret_key;
                        if(strpos($s_key,'test')){ // set test price
                            if($member_subscription->subscription_plan_id == 17015){
                                $price_id = 'price_1NDhLTFj950B2nH9wUGRDgNb'; // max plus
                            }elseif($member_subscription->subscription_plan_id == 19950){
                                $price_id = 'price_1NDhKTFj950B2nH9zob0XTJk'; // max pro
                            }elseif($member_subscription->subscription_plan_id == 19953){
                                $price_id = 'price_1NDhJLFj950B2nH9yQypbajG'; // max advance
                            }
                        }else{  // set live price
                            if($member_subscription->subscription_plan_id == 17015){
                                $price_id = 'price_1NDhI2Fj950B2nH9UPwyibEE'; // live 
                            }elseif($member_subscription->subscription_plan_id == 19950){
                                $price_id = 'price_1NDhGrFj950B2nH9UaQSVoJb'; // max pro
                            }elseif($member_subscription->subscription_plan_id == 19953){
                                $price_id = 'price_1NDhFSFj950B2nH9M7AzdIfh'; // max advance
                            }
                        }
                        if(isset($customer->subscriptions)){
                            $records = $customer->subscriptions->data;
                            if(count($records) == 0){
                                if($price_id != ''){
                                    $sub_args = array(
                                    ...
                                    $sub = Subscription::create($sub_args);
                                }
                            }
                        }else{
                            if($price_id != ''){
    ...
    public function process_payment( $payment_id = 0, $subscription_id = 0 ) {
        ...
        if( isset( $_POST['payment_intent_id' ] ) ){
            ...
            if( !empty( $intent->customer ) ){
                ...
                $cust = Customer::retrieve($intent->customer);
                $s_key = $this->secret_key;
                $trial_sub = false;
                if(strpos($s_key,'test')){ // set test price
                    if($payment->subscription_id == 12039 || $same_plan == 12039){
                        $price_id = 'price_1NDgvnFj950B2nH9gsCs44jN'; // max advance
                    }elseif ($payment->subscription_id == 11984 || $same_plan == 11984) {
                        $price_id = 'price_1NDgvEFj950B2nH9ZT3aWmrX'; // max pro
                    }elseif($payment->subscription_id == 7231 || $same_plan == 7231){
                        $price_id = 'price_1NDguDFj950B2nH9vNa5uev2'; // max plus
                    }elseif($payment->subscription_id == 17015 || $same_plan == 17015){
                        $price_id = 'price_1NDhLTFj950B2nH9wUGRDgNb'; // max plus
                    }elseif($payment->subscription_id == 19950 || $same_plan == 19950){
                        $price_id = 'price_1NDhKTFj950B2nH9zob0XTJk'; // max pro trial
                    }elseif($payment->subscription_id == 19953 || $same_plan == 19953){
                        $price_id = 'price_1NDhJLFj950B2nH9yQypbajG'; // max advance
                    }elseif($payment->subscription_id == 22875 || $same_plan == 22875){
                        $price_id = 'price_1NDhTvFj950B2nH9RWleUeOj'; // max plus
                        $trial_sub = true;
                    }elseif ($payment->subscription_id == 22874 || $same_plan == 22874) {
                        $price_id = 'price_1NDhUrFj950B2nH9il8N2fof'; // max pro
                        $trial_sub = true;
                    }elseif($payment->subscription_id == 22873 || $same_plan == 22873){
                        $price_id = 'price_1NDhVjFj950B2nH9PkLSjyfY'; // max advance
                        $trial_sub = true;
                    }
                }else{  // set live price
                    if($payment->subscription_id == 12039 || $same_plan == 12039){
                        // $price_id = 'price_1KRa5YFj950B2nH9AkGUs8mj'; // max advance - old
                        $price_id = 'price_1NDgrJFj950B2nH9zovwWZmF'; // max advance
                    }elseif ($payment->subscription_id == 11984 || $same_plan == 11984) {
                        $price_id = 'price_1NDgsFFj950B2nH9ze7mCHCM'; // max pro
                    }elseif($payment->subscription_id == 7231 || $same_plan == 7231){
                        $price_id = 'price_1NDgt7Fj950B2nH9RaB0F1ik'; // max plus
                    }elseif($payment->subscription_id == 17015 || $same_plan == 17015){
                        $price_id = 'price_1NDhI2Fj950B2nH9UPwyibEE'; // live 
                    }elseif($payment->subscription_id == 19950 || $same_plan == 19950){
                        $price_id = 'price_1NDhGrFj950B2nH9UaQSVoJb'; // max pro trial
                    }elseif($payment->subscription_id == 19953 || $same_plan == 19953){
                        $price_id = 'price_1NDhFSFj950B2nH9M7AzdIfh'; // max advance
                    }elseif($payment->subscription_id == 22875 || $same_plan == 22875){
                        $price_id = 'price_1NDhOzFj950B2nH9DIElDbZV'; // max plus
                        $trial_sub = true;
                    }elseif ($payment->subscription_id == 22874 || $same_plan == 22874) {
                        $price_id = 'price_1NDhR0Fj950B2nH9i7SnQ6Wv'; // max pro
                        $trial_sub = true;
                    }elseif($payment->subscription_id == 22873 || $same_plan == 22873){
                        $price_id = 'price_1NDhSEFj950B2nH9x9u5CCN3'; // max advance
                        $trial_sub = true;
                    }
                }
                $pm = $intent->payment_method;
                if(isset($cust->subscriptions)){
                    $records = $cust->subscriptions->data;
                    if(count($records) == 0){
                        if($price_id != ''){
                            if($trial_sub){
                                $sub_args = array(
    ...
    public function create_payment_intent(){
        ...
        if(strpos($s_key,'test')){ // set test price
            if($subscription_plan->id == 12039 || $same_plan == 12039){
                $price_id = 'price_1NDgvnFj950B2nH9gsCs44jN'; // max advance
            }elseif ($subscription_plan->id == 11984 || $same_plan == 11984) {
                $price_id = 'price_1NDgvEFj950B2nH9ZT3aWmrX'; // max pro
            }elseif($subscription_plan->id == 7231 || $same_plan == 7231){
                $price_id = 'price_1NDguDFj950B2nH9vNa5uev2'; // max plus
            }elseif($subscription_plan->id == 22873 || $same_plan == 22873){
                $price_id = 'price_1NDhVjFj950B2nH9PkLSjyfY'; // max advance
            }elseif ($subscription_plan->id == 22874 || $same_plan == 22874) {
                $price_id = 'price_1NDhUrFj950B2nH9il8N2fof'; // max pro
            }elseif($subscription_plan->id == 22875 || $same_plan == 22875){
                $price_id = 'price_1NDhTvFj950B2nH9RWleUeOj'; // max plus
            }
        }else{  // set live price
            if($subscription_plan->id == 12039 || $same_plan == 12039){
        ...
        if( is_user_logged_in() ){
            ...
            if($customer){ // Teqtop custom code 01-dec
                $cid = $customer->id;
                $cdata = Customer::retrieve( $cid );
                if(isset($cdata->subscriptions)){
                    $records = $cdata->subscriptions->data;
                    if(count($records) > 0){
                        $single_sub = $records[0]->id;
                        $items_s = $records[0]->items->data[0]->id;                    
                    }
                }          
            }else{ // Teqtop custom code 01-dec
                $customer = $this->create_customer();
            }
        ...
        if($price_id != '' && $cust_id != ''){
            if($new){
                $sub_args = array(
                    'customer'  => $cust_id,
                    'default_payment_method' => $intent->id,
                    'items' => array( 
                            array( 
                                "price" => $price_id, 
                            ), 
            ...
            if($single_sub != '' && $items_s != ''){
                // $sdata = \Stripe\Subscription::cancel($single_sub);
                $sdata = Subscription::update($single_sub, array(
    ...
    /**
     * Create a Setup Intent for an Update Payment Method call
     *
     * Hook: wp_ajax_pms_update_payment_method, wp_ajax_nopriv_pms_update_payment_method
     *
     * @return json
     */
    public function create_update_payment_method_setup_intent(){
    ...
    public function update_customer_payment_method( $member_subscription ){
        ...
        try {
            ...
            //check if this payment method is already attached to the customer, if not, add it
            $customer_payment_methods = \Stripe\PaymentMethod::all( array( 'customer' => $customer->id, 'type' => 'card' ) );
            $attach_card              = true;

            if( !empty( $customer_payment_methods['data'] ) ){
                foreach( $customer_payment_methods['data'] as $existing_payment_method ){
                    if( $payment_method->id == $existing_payment_method->id ){
                        $attach_card = false;
                        break;
                    }
                }
