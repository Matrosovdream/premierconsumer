...
Class PMS_IN_Stripe {
    ...
    private function load_dependencies() {
        ...
        // Admin page
        // TODO: need to figure out how this will work when we think about adding connect in the free version in the near future
        ...
        if( !file_exists( PMS_PLUGIN_DIR_PATH . 'includes/gateways/stripe/admin/functions-admin-connect.php' ) && file_exists( PMS_IN_STRIPE_PLUGIN_DIR_PATH . 'includes/stripe-connect/admin/functions-admin-connect.php' ) )
            include PMS_IN_STRIPE_PLUGIN_DIR_PATH . 'includes/stripe-connect/admin/functions-admin-connect.php';
        ...
        if( !file_exists( PMS_PLUGIN_DIR_PATH . 'includes/gateways/stripe/functions-connect.php' ) && file_exists( PMS_IN_STRIPE_PLUGIN_DIR_PATH . 'includes/stripe-connect/functions-connect.php' ) )
            include PMS_IN_STRIPE_PLUGIN_DIR_PATH . 'includes/stripe-connect/functions-connect.php';
        ...
        if( !file_exists( PMS_PLUGIN_DIR_PATH . 'includes/gateways/stripe/class-payment-gateway-stripe-connect.php' ) && file_exists( PMS_IN_STRIPE_PLUGIN_DIR_PATH . 'includes/stripe-connect/class-payment-gateway-stripe-connect.php' ) )
            include PMS_IN_STRIPE_PLUGIN_DIR_PATH . 'includes/stripe-connect/class-payment-gateway-stripe-connect.php';
        ...
        if( pms_in_stripe_connect_payment_request_enabled() ){

            if( !file_exists( PMS_PLUGIN_DIR_PATH . 'includes/gateways/stripe/apple-pay/functions-apple-pay.php' ) && file_exists( PMS_IN_STRIPE_PLUGIN_DIR_PATH . 'includes/stripe-connect/apple-pay/functions-apple-pay.php' ) )
                include PMS_IN_STRIPE_PLUGIN_DIR_PATH . 'includes/stripe-connect/apple-pay/functions-apple-pay.php';
    ...
    public function enqueue_front_end_scripts() {
        ...
        if( $active_stripe_gateway == 'stripe_connect' ){
            wp_enqueue_script( 'pms-stripe-script', PMS_IN_STRIPE_PLUGIN_DIR_URL . 'assets/js/front-end-connect.js', array('jquery'), PMS_IN_STRIPE_VERSION );
            ...
            $pms_stripe_script_vars['stripe_payment_request'] = pms_in_stripe_connect_payment_request_enabled();
            $pms_stripe_script_vars['stripe_account_country'] = pms_in_stripe_connect_get_account_country();
            $pms_stripe_script_vars['pms_active_currency']    = strtolower( pms_get_active_currency() );

        } else
            wp_enqueue_script( 'pms-stripe-script', PMS_IN_STRIPE_PLUGIN_DIR_URL . 'assets/js/front-end.js?t='.time(), array('jquery'), PMS_IN_STRIPE_VERSION );
