...
function pms_in_msu_member_subscription_list_table_add_new_button( $which, $member, $existing_subscriptions ) {
    ...
    if( $which == 'bottom' ) {
        ...
        if( ( $subscription_groups_count > 1 && count( $member->subscriptions ) < $subscription_groups_count ) ) {
            echo '<a href="' . esc_url( add_query_arg( array( 'page' => 'pms-members-page', 'subpage' => 'add_subscription', 'member_id' => $member->user_id ), admin_url( 'admin.php' ) ) ) . '" class="button-secondary" style="display: inline-block;">' . esc_html__( 'Add New Subscription', 'paid-member-subscriptions' ) . '</a>';
        }
