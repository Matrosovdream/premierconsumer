...
<div id="pms-settings-tax" class="pms-tab <?php echo ( $active_tab == 'tax' ? 'tab-active' : '' ); ?>">
    ...
    <div id="enable-tax">
        ...
        <div class="pms-form-field-wrapper">
            <label class="pms-form-field-label" for="enable-tax"><?php esc_html_e( 'Enable Tax Rates', 'paid-member-subscriptions' ) ?></label>

            <p class="description"><input type="checkbox" id="enable-tax" name="pms_tax_settings[enable_tax]" value="1" <?php echo ( isset( $options['enable_tax'] ) ? checked($options['enable_tax'], '1', false) : '' ); ?> /><strong><?php esc_html_e( 'Enable taxes and tax calculations on all subscription plan purchases.', 'paid-member-subscriptions' ); ?></strong></p>
        </div>
    ...
    <div id="tax-options">
        ...
        <h4 class="pms-subsection-title"><?php esc_html_e( 'Tax Options', 'paid-member-subscriptions' ); ?></h4>

        <div class="pms-form-field-wrapper">
            <label class="pms-form-field-label"><?php esc_html_e( 'Prices entered with tax', 'paid-member-subscriptions' ) ?></label>
        ...
        <div class="pms-form-field-wrapper">
            <label class="pms-form-field-label" for="default-billing-country"><?php esc_html_e( 'Default Billing Country', 'paid-member-subscriptions' ) ?></label>
        ...
        <div class="pms-form-field-wrapper">
            <label class="pms-form-field-label" for="eu-vat-enable"><?php esc_html_e( 'Enable EU VAT', 'paid-member-subscriptions' ) ?></label>
            ...
            <p class="description">
                ...
                <?php echo wp_kses_post( __( 'Enable EU VAT on subscription purchases. <br> Your customers will also be able to provide a VAT ID in order to be exempt of paying the vat.<br>', 'paid-member-subscriptions' ) ); ?>
                <?php esc_html_e( 'The plugin already includes the VAT rates for EU countries so you don\'t have to add them, but you can overwrite them below if necessary.', 'paid-member-subscriptions' ); ?>
            </p>
        </div>

        <div class="pms-form-field-wrapper" id="eu-merchant__wrapper">
            <label class="pms-form-field-label" for="merchant-vat-country"><?php esc_html_e( 'Merchant VAT Country', 'paid-member-subscriptions' ) ?></label>
    ...
    <div id="tax-rates">
        ...
        <h4 class="pms-subsection-title"><?php esc_html_e('Tax Rates', 'paid-member-subscriptions'); ?></h4>
        ...
        <div id="custom-tax-rates">
            ...
            <table id="pms_custom_tax_rates" class="wp-list-table widefat">
                ...
                <?php foreach( $tax_rates as $i => $tax_rate ): ?>
                    <tr id="pms_tax_rate_row_<?php echo esc_attr( $tax_rate['id'] ); ?>" <?php echo ( ( ($i % 2) == 0 ) ? 'class="alternate"' : '' ); ?>>
                        ...
                        <td width="25px"><a title="<?php esc_html_e('Delete','paid-member-subscriptions'); ?>" href="" class="pms-tax-rate-remove alignright" data-id="<?php echo esc_attr( $tax_rate['id'] ); ?>"><span class="dashicons dashicons-no-alt"></span></a></td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
            </table>
            ...
            <div style="float: right;">
                <strong><a href="<?php
                    ...
                    ?>" class="button"><?php esc_html_e('Export Tax Rates', 'paid-member-subscriptions'); ?></a></strong>
                ...
                <strong><a href="<?php
                    ...
                    ?>" class="button" onclick="if(!confirm('<?php esc_html_e('Are you sure? This will delete all tax rates from the database', 'paid-member-subscriptions'); ?>')){return false;}"><?php esc_html_e('Clear Tax Rates', 'paid-member-subscriptions'); ?></a></strong>
            </div>
            <br/>

        <?php else: ?>
            <div id="custom-tax-rates"><strong><?php esc_html_e('No custom tax rates have been set. You can add some by uploading a CSV file below.', 'paid-member-subscriptions'); ?></strong></div>
        <?php endif; ?>

        <div class="pms-form-field-wrapper">

            <label class="pms-form-field-label" for="default-tax-rate"><?php esc_html_e('Default Tax Rate (%)', 'paid-member-subscriptions'); ?></label>
            ...
            <p class="description"><?php esc_html_e('Enter a tax percentage. Customers not in a specific tax rate (defined above) will be charged this rate.', 'paid-member-subscriptions'); ?></p>

        </div>

        <div class="pms-form-field-wrapper">

            <label class="pms-form-field-label" for="tax-rates-csv"><?php esc_html_e('Upload Tax Rates', 'paid-member-subscriptions'); ?></label>

            <input type="file" id="tax-rates-csv" name="pms_tax_rates_csv" />

            <p class="description"><?php esc_html_e('Upload Tax Rates via a CSV file. Use this to select a csv file, then to upload, just click "Save Settings" button.', 'paid-member-subscriptions')?> </p>
            <p class="description"><a href="<?php echo esc_url( PMS_IN_TAX_PLUGIN_DIR_URL . 'sample-data/sample_tax_rates.csv' ); ?>"> <?php esc_html_e('Download this sample CSV Tax Rates file', 'paid-member-subscriptions')?></a> <?php esc_html_e(' and modify it by adding your required tax rates.','paid-member-subscriptions')?></p>
