...
Class PMS_Shortcodes {
    ...
    public static function subscriptions_form( $atts ) {
        ...
        if( is_user_logged_in() ) {
            ...
            if( $member->is_member() ) {
                return wp_redirect(home_url()."/dashboard/subscriptions"); // TEQTOP Custom Code
    ...
    public static function member_account( $args ) {
        $slang = apply_filters( 'wpml_current_language', null ); // TEQTOP Custom Code
        ...
        if ( $args['show_tabs'] === true ) {
            ...
            if ( empty( $active_tab ) || !isset( $tabs[$active_tab] ) ){
                ...
                $active_tab = 'dashboard'; // TEQTOP Custom Code
            }
            ...
            <?php if( !empty( $tabs ) && is_array( $tabs ) ){
                ...
                $nav = '<nav class="pms-account-navigation">';
                    $nav .= '<ul>';
                    foreach( $tabs as $slug => $name ){ 
                        $nav .= '<li class="pms-account-navigation-link pms-account-navigation-link--'.esc_attr( $slug ).'">';
                        $nav .= '<a class="custom-pms-links" href="'.pms_account_get_tab_url( $slug, $account_page ).'">'.esc_html( $name ).'</a>';
                        $nav .= '</li>';
                    }
                    $nav .= '</ul>';
                $nav .= '</nav>';
        ...
        $member = pms_get_member( $user_id );
        $output = '';
        echo $output0 = '<div class="container-fluid dashboard-container">';

        echo $output1 = '<div class="row">';
        if($slang == "es"){
            $header_content = '<div class="col-md-12"><a href="'.$account_page1.'"><h1 class="dash_title">Tablero de mandos</h1></a>';
        }else{
            $header_content = '<div class="col-md-12"><a href="'.$account_page1.'"><h1 class="dash_title">Dashboard</h1></a>';
        }   
        
        $header_content .= $nav;
        $header_content .= '</div>';

        echo $output_head = $header_content;
        echo $output_head_end = '</div>';
        ...
        if ( $args['show_tabs'] === true ) {
            ...
            $tabs = apply_filters( 'pms_member_account_tabs', array(
                'subscriptions' => __( 'Subscriptions', 'paid-member-subscriptions' ),
                'profile'       => __( 'Edit Profile', 'paid-member-subscriptions' ),
                'payments'      => __( 'My Account', 'paid-member-subscriptions' ),
            ), $args );
        ...
        /* TEQTOP - Custom Code Start */
        $credit_plans = array("12039","11984","7231","17015","19950","19953","22873","22874","22875");
        $show_banner = false;
        if(!pms_is_member_of_plan( $credit_plans, pms_get_current_user_id())){
            $show_banner = true;
        }
        echo $output_tab_start = '<div class="container">';
        echo $output_row = '<div class="row">';
        if($show_banner){
            echo $output4 = '<div class="col-md-9">';
        }else{
            echo $output4 = '<div class="col-md-12">';
        }
        /* TEQTOP - Custom Code End */

        if ( $active_tab == 'dashboard' ) { // TEQTOP Custom Code Condition
            include 'views/shortcodes/view-shortcode-report.php';         
        ...
        }else if ( $active_tab == 'documents' ) { // upload document TEQTOP Custom Code
            include 'views/shortcodes/view-shortcode-upload-documents.php';
        }
        ...
        if(!pms_is_member_of_plan( $credit_plans, pms_get_current_user_id())){
            if($slang == "es"){
                $ad_link = home_url()."/monitoreo-de-credito";
                $btn_text = "Inicia ahora";
                $h3_text = "Programa de Protección de su Identidad";
                $desc_text = "Enrólese en monitoreo de crédito y lleve el control de su progreso.";
            }else{
                $ad_link = home_url()."/credit-monitoring/?lang=en";
                $btn_text = "Start Now";
                $h3_text = "Identity Protection Program";
                $desc_text = "Enroll in credit monitoring and keep track of your progress.";
            }
            $advertise_banner = '<div class="col-md-3"><section class="dashboard-banner">';
            $advertise_banner .=  '<div class="container"><div class="row"><div class="col-md-12">';
            $advertise_banner .=  '<div class="dashboard-content">';
            $advertise_banner .=  '<h3>'.$h3_text.'</h3>';
            $advertise_banner .=  '<p>'.$desc_text.'</p>';
            $advertise_banner .=  '<div class="button-outer"><a href="'.$ad_link.'">'.$btn_text.'</a></div>';
            $advertise_banner .= '</div></div></div></div>';
            $advertise_banner .= '</section></div>';
            echo $advertise_banner;
        }
        if ( is_active_sidebar( 'upgrade-plan' ) && pms_is_member_of_plan( $credit_plans, pms_get_current_user_id() )){
            echo '<div class="col-md-4"><div class="side-cols side-col2 upgrade-plan_wid">';
            $content_upgrade = dynamic_sidebar( 'upgrade-plan' );
            echo '</div></div>';
        }
        $plan_active = "";
        $member = pms_get_member( pms_get_current_user_id() );
        ...
        if(isset($member->subscriptions) && !empty($member->subscriptions)){
            ...
            $plan_active = "1";
            if($subscription_plan_id == "7231" || $same_plan == "7231" || $subscription_plan_id == "22875"){ 
                echo '<div class="side-cols side-col1">';
                if ( is_active_sidebar( 'max-plus' ) ){
                    $side_max = dynamic_sidebar( 'max-plus' );
                }
                echo '</div>';
              }elseif($subscription_plan_id == "7769" || $subscription_plan_id == "11984" || $subscription_plan_id == "22874" || $same_plan == "11984"){ 
                echo '<div class="side-cols side-col1">';
                if ( is_active_sidebar( 'max-pro' ) ){
                    $side_pro = dynamic_sidebar( 'max-pro' );
                }
                echo '</div>';
              }elseif($subscription_plan_id == "7770" || $subscription_plan_id == "12039" || $subscription_plan_id == "22873" || $same_plan == "12039"){
                echo '<div class="side-cols side-col1">';
                if ( is_active_sidebar( 'max-advanced' ) ){
                    $side_advance = dynamic_sidebar( 'max-advanced' ); 
                }
                echo '</div>';
            }
            echo "</div>";
        }
        echo $output_row_end = '</div>';
        echo $output6 = '</div>';
        echo $output7 = '</div>';
    ...
    public static function pms_wp_login_form( $args = array() ) {
        ...
        if($slang == "es"){
            $defaults = array(
                ...
                'label_username' => __( 'Nombre de usuario o dirección de correo electrónico', 'paid-member-subscriptions' ),
                'label_password' => __( 'Contraseña', 'paid-member-subscriptions' ),
                'label_remember' => __( 'Recuérdame', 'paid-member-subscriptions' ),
                'label_log_in'   => __( 'Iniciar sesión', 'paid-member-subscriptions' ),
    ...
    public static function login_form_bottom( $string, $args ) {
        ...
        if ( !empty($args['register']) ) {
            ...
            /* TEQTOP - Custom Code Start */
            if($slang == "es"){
                $string .= '<a class="register" href="' . esc_url($args['register']) . '">' . apply_filters('pms_login_register_text', __('Regístrese', 'paid-member-subscriptions')) . '</a>';
            }else{
                $string .= '<a class="register" href="' . esc_url($args['register']) . '">' . apply_filters('pms_login_register_text', __('Register', 'paid-member-subscriptions')) . '</a>';
            }
            /* TEQTOP - Custom Code End */
        ...
        if ( !empty($args['lostpassword']) ) {
            ...
            /* TEQTOP - Custom Code Start */
            if($slang == "es"){
                $string .= '<a class="lostpassword" href="' . esc_url($args['lostpassword']) . '">' . apply_filters('pms_login_lostpass_text', __('¿Perdiste tu contraseña?', 'paid-member-subscriptions')) . '</a>';
            }else{
                $string .= '<a class="lostpassword" href="' . esc_url($args['lostpassword']) . '">' . apply_filters('pms_login_lostpass_text', __('Lost your password?', 'paid-member-subscriptions')) . '</a>';
            }
            /* TEQTOP - Custom Code End */
    ...
    public static function recover_password_form( $atts ){
        ...
        if($slang == "es"){
            ...
            if( isset( $_POST['pms_username_email'] ) && count( pms_errors()->get_error_codes() ) == 0 ) {
                return apply_filters( 'pms_recover_password_confirmation_link_message', '<p>' . __( 'Por favor, compruebe su correo electrónico para ver el enlace de confirmación.', 'paid-member-subscriptions' ) . '</p>' );
            }
        }else{
            // If entered username or email is valid, display a message to the user and email confirmation link
            if( isset( $_POST['pms_username_email'] ) && count( pms_errors()->get_error_codes() ) == 0 ) {
