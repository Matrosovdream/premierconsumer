...
<form id="pms_recover_password_form" class="pms-form" method="post">
    ...
    <?php
        if($slang == "es"){
            $pms_recover_notification = '<p>' . __( 'Por favor, introduzca su nombre de usuario o dirección de correo electrónico.', 'paid-member-subscriptions' );
            $pms_recover_notification .= '<br/>'.__( 'Recibirás un enlace para crear una nueva contraseña por correo electrónico.', 'paid-member-subscriptions' ).'</p>';
        }else{
    ...
    <ul class="pms-form-fields-wrapper">
        ...
        <li class="pms-field <?php echo ( !empty( $field_errors ) ? 'pms-field-error' : '' ); ?>">
            <?php if($slang == "es"){ ?>
                <label for="pms_username_email"><?php echo esc_html( apply_filters( 'pms_recover_password_form_label_username_email', __( 'Nombre de usuario o correo electrónico', 'paid-member-subscriptions' ) ) ); ?></label>
            <?php }else{ ?>
    ...
    <?php if($slang == "es"){ ?>
        <input type="submit" name="submit" value="<?php esc_attr_e( 'Reiniciar la contraseña', 'paid-member-subscriptions' ); ?>"/>
    <?php }else{ ?>
