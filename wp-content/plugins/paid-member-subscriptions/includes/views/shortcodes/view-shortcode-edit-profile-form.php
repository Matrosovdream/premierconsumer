...
<form id="pms_edit-profile-form" class="pms-form" method="POST">
    ...
    <ul class="pms-form-fields-wrapper">
        ...
        <li class="pms-field pms-user-login-field">
            <?php if($site_lang == "es"){ ?>
                <label for="pms_user_login"><?php echo esc_html( apply_filters( 'pms_edit_profile_form_label_user_login', __( 'Nombre de usuario *', 'paid-member-subscriptions' ) ) ); ?></label>
            <?php }else{ ?>
        ...
        <li class="pms-field pms-first-name-field <?php echo ( !empty( $field_errors ) ? 'pms-field-error' : '' ); ?>">
            <?php if($site_lang == "es"){ ?>
                <label for="pms_first_name"><?php echo esc_html( apply_filters( 'pms_register_form_label_first_name', __( 'Nombre de pila', 'paid-member-subscriptions' ) ) ); ?></label>
            <?php }else{ ?>
        ...
        <li class="pms-field pms-pass1-field <?php echo ( !empty( $field_errors ) ? 'pms-field-error' : '' ); ?>">
            <?php if($site_lang == "es"){ ?>
                <label for="pms_pass1"><?php echo esc_html( apply_filters( 'pms_register_form_label_pass1', __( 'Contraseña', 'paid-member-subscriptions' ) ) ); ?></label>
            <?php }else{ ?>
        ...
        <li class="pms-field pms-pass2-field <?php echo ( !empty( $field_errors ) ? 'pms-field-error' : '' ); ?>">
            <?php if($site_lang == "es"){ ?>
                <label for="pms_pass2"><?php echo esc_html( apply_filters( 'pms_register_form_label_pass2', __( 'Repita la contraseña', 'paid-member-subscriptions' ) ) ); ?></label>
            <?php }else{ ?>
        ...
        <li>
            <?php if($site_lang == "es"){ ?>
                <input name="pms_edit_profile" type="submit" value="<?php echo esc_attr( apply_filters( 'pms_edit_profile_form_submit_text', __( 'Editar perfil', 'paid-member-subscriptions' ) ) ); ?>" />
            <?php } else { ?>
