...
$cim_subscription_plans = array('16637','16638','19780');
$mim_subscription_plans = array('16336','16370','19781');
$cim_className = $cim_login_link = "";
$mim_className = $mim_login_link = "";
if(pms_is_member_of_plan( $cim_subscription_plans, $user_id )){
	$cim_className = "Prefiling Credit Counseling";
    $cim_login_link = 'http://www.counselinginmotion.com/studentlogin.php';
}
if(pms_is_member_of_plan( $mim_subscription_plans, $user_id )){
...
foreach( $subscriptions as $subscription ) :
	...
	$plan_idd = $subscription_plan->id;
	if(in_array($plan_idd, $mim_subscription_plans)){
		$cim_className = $cim_login_link = "";
		$mim_className = "Debtor";
    	$mim_login_link = 'https://www.moneyinmotion.us/studentlogin.php';
	}
	if(in_array($plan_idd, $cim_subscription_plans)){
		$mim_className = $mim_login_link  = "";
		$cim_className = "Prefiling Credit Counseling";
    	$cim_login_link = 'http://www.counselinginmotion.com/studentlogin.php';
	}
	...
	<table class="pms-table">
		<tbody>
			...
			<?php if($site_lang == "es") { ?>
				<tr class="pms-account-subscription-details-table">
				    <th><?php esc_html_e( 'Plan de suscripción', 'paid-member-subscriptions' ); ?></th>
				    <th><?php esc_html_e( 'Status', 'paid-member-subscriptions' ); ?></th>
				    <th><?php esc_html_e( 'Fecha de inicio', 'paid-member-subscriptions' ); ?></th>
				    <?php if( empty( $subscription->billing_next_payment ) ) : ?>
				    	<?php if(($cim_login_link != "" && $cim_className != "") || ($mim_login_link != "" && $mim_className != "")){ ?>
				    		<th><?php esc_html_e( 'Enlace de acceso a la clase', 'paid-member-subscriptions' ); ?></th>
				    	<?php }else{ ?>
				    		<th><?php esc_html_e( 'Próximo pago', 'paid-member-subscriptions' ); ?></th>
				    	<?php } ?>
				    <?php endif; ?>
				    <?php if( $subscription->is_trial_period() ): ?>
				    <th><?php echo esc_html( ucfirst( date_i18n( get_option('date_format'), strtotime( $subscription->trial_end ) ) ) ); ?></th>
				    <?php endif; ?>
				    <?php if( ! empty( $subscription->billing_next_payment ) && $subscription->status == 'active' ): ?>
				    <th><?php esc_html_e( 'Próximo pago', 'paid-member-subscriptions' ); ?></th>
				    <?php endif; ?>
				    <th><?php esc_html_e( 'Acciones', 'paid-member-subscriptions' ); ?></th>
			    </tr>
			<?php } else { ?>
				<tr class="pms-account-subscription-details-table">
				    <th><?php esc_html_e( 'Subscription Plan', 'paid-member-subscriptions' ); ?></th>
				    <th><?php esc_html_e( 'Status', 'paid-member-subscriptions' ); ?></th>
				    <th><?php esc_html_e( 'Start Date', 'paid-member-subscriptions' ); ?></th>
				    <?php if( empty( $subscription->billing_next_payment ) ) : ?>
				    	<?php if(($cim_login_link != "" && $cim_className != "") || ($mim_login_link != "" && $mim_className != "")){ ?>
				    		<th><?php esc_html_e( 'Class Login Link', 'paid-member-subscriptions' ); ?></th>
				    	<?php }else{ ?>
				    		<th><?php esc_html_e( 'Next Payment', 'paid-member-subscriptions' ); ?></th>
				    	<?php } ?>
				    <?php endif; ?>
				    <?php if( $subscription->is_trial_period() ): ?>
				    <th><?php echo esc_html( ucfirst( date_i18n( get_option('date_format'), strtotime( $subscription->trial_end ) ) ) ); ?></th>
				    ...
				    <th><?php esc_html_e( 'Next Payment', 'paid-member-subscriptions' ); ?></th>
				    <?php endif; ?>
				    <th><?php esc_html_e( 'Actions', 'paid-member-subscriptions' ); ?></th>
			    </tr>
			<?php } ?>
            ...
            <tr class="pms-account-subscription-details-table">
                ...
                <?php if( empty( $subscription->billing_next_payment ) ) : ?>
                	<?php if($cim_login_link != "" && $cim_className != ""){ ?>
                		<td>
                			<a href="<?php echo $cim_login_link; ?>" target="_blank"><?php echo $cim_className; ?> Education Class</a>
                		</td>
                	<?php }elseif($mim_login_link != "" && $mim_className != ""){ ?>
                		<td>
                			<a href="<?php echo $mim_login_link; ?>" target="_blank"><?php echo $mim_className; ?> Education Class</a>
                		</td>
                	<?php } else { ?>
		                <td>
		                	<?php echo ( ! empty( $subscription->expiration_date ) ? esc_html( ucfirst( date_i18n( get_option('date_format'), strtotime( $subscription->expiration_date ) ) ) ) : esc_html__( 'Unlimited', 'paid-member-subscriptions' ) ); ?>
