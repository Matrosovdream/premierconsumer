...
<form id="pms_new_password_form" class="pms-form" method="post">
    ...
    <?php
    if($slang == "es"){
        $pms_newpass_notification = '<p>' . __( 'Por favor, introduzca su nueva contraseña.', 'paid-member-subscriptions' ) . '</p>';
    }else{
    ...
    <ul class="pms-form-fields-wrapper">
        ...
        <li class="pms-field">
            <?php if($slang == "es"){ ?>
                <label for="pms_new_password"><?php echo esc_html( apply_filters( 'pms_recover_password_form_label_new_password', __( 'Contraseña', 'paid-member-subscriptions' ) ) ); ?></label>
            <?php }else{ ?>
        ...
        <li class="pms-field <?php echo ( !empty( $field_errors ) ? 'pms-field-error' : '' ); ?>">
            <?php if($slang == "es"){ ?>
                <label for="pms_new_password"><?php echo esc_html( apply_filters( 'pms_recover_password_form_label_new_password', __( 'Repita la contraseña', 'paid-member-subscriptions' ) ) ); ?></label>
            <?php }else{ ?>
