...
Class PMS_Form_Handler {
    ...
    public static function get_request_member_data( $user_id = 0 ) {
        ...
        if(in_array(16336,$subscription_plans) && isset($_POST['person']) && $_POST['person'] != ""){
            $member_data['person'] = $_POST['person'];
            $member_data['income'] = $_POST['yearly_income'];
        }
    ...
    public static function get_redirect_url() {
        ...
        $url      = '';
        /* TEQTOP - Custom Code Start */
        $all_data = isset($_POST) ? $_POST : [];
        if(count($all_data) > 0){
            $plans = isset($all_data['subscription_plans']) ? $all_data['subscription_plans'] : [];
            $credit_monitoring_plans = array("12039","11984","7231","17015","19950","19953","22875","22874","22873");
            $creditor_plans = array("16637","16638","19780");
            $debtor_plans = array("16336","19781","16370");
            $slang = apply_filters( 'wpml_current_language', null ); // Custom Code
            if(in_array($plans, $creditor_plans)){
                if($slang == "es"){
                    $url = "https://www.premierconsumer.org/pre-filing-gracias/?lang=es";
                }else{
                    $url = "https://www.premierconsumer.org/pre-filing-thank-you/";
                }
            }
            if(in_array($plans, $debtor_plans)){
                if($slang == "es"){
                    $url = "https://www.premierconsumer.org/post-filing-gracias/?lang=es";
                }else{
                    $url = "https://www.premierconsumer.org/post-filing-thank-you/";
                }
            }
            if(in_array($plans, $credit_monitoring_plans)){
                if($slang == "es"){
                    $url = "https://www.premierconsumer.org/monitoreo-de-credito-gracias/?lang=es";
                }else{
                    $url = "https://www.premierconsumer.org/credit-monitoring-thank-you/";
                }
            }
        }
        /* TEQTOP - Custom Code End */
        ...
        switch( $location ) {
            ...
            case 'register':
                ...
                if($url == ""){ // TEQTOP custom code - condition
    ...
    public static function recover_password_form() {
        ...
        if( isset( $_POST['pms_username_email'] ) ) {
            ...
            if( empty( $username_email ) ){
                if($slang == "es"){  // TEQTOP Custom Code
                    pms_errors()->add( 'pms_username_email', __( 'Por favor, introduzca un nombre de usuario o una dirección de correo electrónico.', 'paid-member-subscriptions' ) );
                }else{
                ...
                if ( !is_email($username_email) ) {
                    ...
                    }else { // TEQTOP Custom Code
                        if($slang == "es"){
                            pms_errors()->add('pms_username_email',__( 'El nombre de usuario introducido no existe. Por favor, inténtelo de nuevo.', 'paid-member-subscriptions'));
                        }else{
                            pms_errors()->add('pms_username_email',__( 'The entered username doesn\'t exist. Please try again.', 'paid-member-subscriptions'));
                        }
                    }
                }
                ...
                if ( is_email( $username_email ) ){
                    if ( email_exists($username_email) ) {
                        ...
                        // TEQTOP Custom Code
                        if($slang == "es"){
                            pms_errors()->add('pms_username_email',__( 'El correo electrónico introducido no se ha encontrado en nuestra base de datos. Por favor, inténtelo de nuevo.', 'paid-member-subscriptions'));
                        }else{
                            pms_errors()->add('pms_username_email',__( 'The entered email wasn\'t found in our database. Please try again.', 'paid-member-subscriptions'));
                        }                        
                    }
                }
            ...
            if ( count( pms_errors()->get_error_codes() ) == 0 ) {
                ...
                if (is_object($user)) {  //user data is set
                    ...
                    //Confirmation link email content - TEQTOP Custom Code
                    if($slang == "es"){
                        $recoveruserMailMessage1 = sprintf(__('Alguien acaba de solicitar el restablecimiento de la contraseña de la siguiente cuenta: <b>%1$s</b><br/><br/>Si se trata de un error, ignora este correo electrónico y no pasará nada.<br/>Para restablecer su contraseña, visite el siguiente enlace: %2$s', 'paid-member-subscriptions'), $username_email, '<a href="' . esc_url(add_query_arg(array('loginName' => urlencode( $requestedUserLogin ), 'key' => $key), pms_get_current_page_url())) . '">' . esc_url(add_query_arg(array('loginName' => urlencode( $requestedUserLogin ), 'key' => $key), pms_get_current_page_url())) . '</a>');
                    }else{
                    ...
                    //Confirmation link email title - TEQTOP Custom Code
                    if($slang == "es"){ 
                        $recoveruserMailMessageTitle1 = sprintf(__('Restablecimiento de la contraseña desde "%s"', 'paid-member-subscriptions'), $blogname = wp_specialchars_decode(get_option('blogname'), ENT_QUOTES));
                    }else{
                    ...
                    if (trim($recoveruserMailMessageTitle1) != '') {
                        ...
                        if ($sent === false){
                            if($slang == "es"){ // TEQTOP Custom Code
                                pms_errors()->add('pms_username_email',__( 'Ha habido un error al intentar enviar el enlace de activación.', 'paid-member-subscriptions'));
                            }else{
    ...
    public static function process_checkout( $user_data = array() ) {
        ...
        if( $payment_response ) {
            ...
            switch( $form_location ) {
                ...
                after_new_subscription($_POST); // TEQTOP - Custom Code
                ...
                // upgrading the subscription - TEQTOP Custom Code
                case 'upgrade_subscription':
                    ...
                    if($api_user_id != ""){
                         update_user_meta($user_id,"old_plan_user_id_".$subscription->subscription_plan_id,$api_user_id,false);
                    }
                    update_user_meta($user_id,"api_user_id","",false);
                    break;
